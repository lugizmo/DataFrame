...
namespace lugizmo {
    ...
    /**
     * @brief Indicates if expensive asserts are enabled for this build.
     */
    [[nodiscard]]
    consteval auto AssertExpensiveEnabled() noexcept -> bool
    {
#if defined(LUGIZMO_DF_ENABLE_ASSERT_EXPENSIVE) && LUGIZMO_DF_ENABLE_ASSERT_EXPENSIVE
        return true;
#else
        return false;
#endif
...
#if defined(LUGIZMO_DF_ENABLE_ASSERT_EXPENSIVE) && LUGIZMO_DF_ENABLE_ASSERT_EXPENSIVE
#define LUGIZMO_ASSERT_EXP(condition, message)                                                                \
    do                                                                                                        \
    {                                                                                                         \
        if(!(condition))                                                                                      \
        {                                                                                                     \
...
#define LUGIZMO_ASSERT_EXP(condition, message)                                                                \
    do                                                                                                        \
    {                                                                                                         \
        [[maybe_unused]] constexpr bool lugizmoAssertExpDisabled = true;                                      \
        (void)lugizmoAssertExpDisabled;                                                                       \
    } while(false)
#endif
